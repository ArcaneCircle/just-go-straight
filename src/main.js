...
function main() {
  ...
  loop.start(function (dt) {
    ...
    if (
      mob.end > -1 &&
      key.isDown(key.ENTER) &&
      (!game_complete || timeout_game_complete <= 0)
    ) {
      if (game_complete) {
        game_complete = false;
    ...
    if (!mob.win && !mob.back_to_beach) {
      var mobSpeed = key.touchMoving ? 150 : 300;
    ...
    if (mob.moving) {
      timer_step += dt;
      if (
        ((mob.back_to_beach || mob.win) && timer_step > 0.09) ||
        (!mob.back_to_beach && !mob.win && timer_step > 0.04)
      ) {
        timer_step = 0;
        var step = {};
        if (mob.step) {
        ...
        if (mob.beach) beach.steps.push(step);
        else jungle.steps.push(step);
      }
    }
    // steps
  ...
  function generate_jungle(path_taken) {
    ...
    if (
      jungle.clone_index == -1 &&
      (jungle.hat || (jungle.sign > -1 && jungle_save.length < 3))
    ) {
    ...
    if (rand_int(11) > 6 && jungle_save.length > 1) {
      ...
      jungle.tileset.forEach(function (tile, index) {
        if (path_taken == 2 && tile == 3) mob.x = index * 160 + 82;
      });
      jungle.tilesetb.forEach(function (tile, index) {
        if (path_taken == 0 && tile == 3) mob.x = index * 160 + 82;
      });
    ...
    if (path_taken == 0) mob.x = index_path_bottom * 160 + 82;
    if (path_taken == 2) mob.x = index_path_top * 160 + 78;
  ...
  function check_collisions() {
    var bounds = {};
    if (mob.beach) bounds = beach.bounds;
    else bounds = jungle.bounds;
  ...
  function clone(obj) {
    ...
    if (Array.isArray(obj)) {
      copy = obj.map(function (elm) {
        return Array.isArray(elm) || (typeof elm === "object" && elm)
          ? clone(elm)
          : elm;
      });
    } else {
      ...
      for (var attr in obj) {
        if (obj.hasOwnProperty(attr)) {
          if (
            ...
            (typeof obj[attr] === "object" && obj[attr])
          ) {
